...
namespace UnitTestProject1
{
    ...
    public class UnitTest1
    {
        ...
        public void GetBonusAnonymous_Test()
        {
            ...
            order.Bonus = delegate (double amount) { return amount > 5 ? 2.0 : 0.0; }; // <- Change to anonymous delegate
            Assert.AreEqual(2.0, order.GetBonus());
        }
        [TestMethod]
        public void GetBonusLambda_Test()
        {
            order.Bonus = x => x / 10; // <- Change to lambda expression
            Assert.AreEqual(4.5, order.GetBonus());

            order.Bonus = x => x > 5 ? 2.0 : 0.0; // <- Change to lambda expression
            Assert.AreEqual(2.0, order.GetBonus());
        }
